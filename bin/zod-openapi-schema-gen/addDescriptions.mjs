...
function findObjectPropertyKey(path) {
...
export function attachDescriptions(node, descriptionValue, exampleValues, importedIdents = new Set(), depth = 0) {
    if (t.isCallExpression(node)) {
        ...
        if (t.isMemberExpression(callee) && callee.property.name === 'array') {
            ...
            if (innerArg) {
                node.arguments[0] = attachDescriptions(innerArg, descriptionValue, exampleValues, importedIdents, depth+1);
            }
            // add a summary at the end 

            if(depth > 0) {
                const keys = innerArg.arguments && innerArg.arguments[0] && innerArg.arguments[0].properties && innerArg.arguments[0].properties.length ? innerArg.arguments[0].properties.map(({key}) => key.name): [];
                console.log(keys)
                ...
                return t.callExpression(t.memberExpression(node, t.identifier('openapi')), [
                    t.objectExpression([
                        t.objectProperty(t.identifier('description'), t.valueToNode(generatedDescription)),
                    ]),
                ]);
            }
